...
class FunctionModificationLensTest : JavaCodeInsightFixtureTestCase() {
    ...
    private fun List<PsiDDItem>.findByPsi(context: IJDDContext, filter: (PsiElement) -> Boolean) =
        find { filter(PsiUtils.getPsiElementFromItem(context, it)!!) }

    private fun List<PsiDDItem>.findLastByPsi(context: IJDDContext, filter: (PsiElement) -> Boolean) =
        findLast { filter(PsiUtils.getPsiElementFromItem(context, it)!!) }

    private fun List<PsiDDItem>.filterByPsi(context: IJDDContext, filter: (PsiElement) -> Boolean) =
